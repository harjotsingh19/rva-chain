...
`$workspace/geth account new --datadir ${DATA_DIR} --password /dev/null > ${workspace}/nodes/${NODE_ID}Info`
...
> Refer to **rva-node1** directory inside **nodes** directory in **shared-files.zip** to see geth and bls folder structure.
...
`$workspace/geth account new --datadir ${DATA_DIR} --password /dev/null > ${workspace}/new-validator/${NODE_ID}Info`
...
`$workspace/geth bls account list --datadir ${DATA_DIR}`
